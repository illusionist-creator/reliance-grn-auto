#!/usr/bin/env python3
"""
Streamlit App for Reliance GRN Automation Workflows
...
st.set_page_config(
    page_title="Reliance GRN Automation",
    page_icon="⚡",
    layout="wide",
...
class RelianceAutomation:
    ...
    def _process_extracted_data(self, extracted_data: Dict, file_info: Dict) -> List[Dict]:
        """Process extracted data from LlamaParse using json_to_excel.py logic"""
        rows = []
        
        # Extract metadata
        metadata = {
            "Supplier Name": self._get_value(extracted_data, ["supplier.name", "supplier_name", "vendor"]),
            "Supplier Address": self._get_value(extracted_data, ["supplier.address", "supplier_address"]),
            "Supplier GSTIN": self._get_value(extracted_data, ["supplier.gstin", "supplier_gstin"]),
            "Supplier Code": self._get_value(extracted_data, ["supplier.code", "supplier_code"]),
            "Goods Receipt Note Number": self._get_value(extracted_data, ["goods_receipt_note_number", "grn_number"]),
            "Goods Receipt Note Date": self._get_value(extracted_data, ["goods_receipt_note_date", "grn_date"]),
            "PO Number": self._get_value(extracted_data, ["po_number", "purchase_order_number"]),
            "PO Date": self._get_value(extracted_data, ["po_date", "purchase_order_date"]),
            "Vendor Invoice Number": self._get_value(extracted_data, ["vendor_invoice_number", "invoice_number"]),
            "Consignee Name": self._get_value(extracted_data, ["consignee.name", "consignee_name"]),
            "Consignee Address": self._get_value(extracted_data, ["consignee.address", "consignee_address"]),
            "Consignee GSTIN": self._get_value(extracted_data, ["consignee.gstin", "consignee_gstin"]),
            "Delivery Address": self._get_value(extracted_data, ["delivery_address", "shipping_address"]),
            "Delivery Number": self._get_value(extracted_data, ["delivery_number"]),
            "Page Number": self._get_value(extracted_data, ["page_number"]),
            "Reason": self._get_value(extracted_data, ["reason"]),
        ...
        # Handle items
        items = extracted_data.get("items", []) or extracted_data.get("product_items", [])
        if not items:
            return rows
        ...
        columns_to_drop = ["reason_short_description", "batch", "ean_number", "vendor_article_no"]
        items_df = items_df.drop(columns=[col for col in columns_to_drop if col in items_df.columns], errors='ignore')
        
        # Rename accepted_qty_mrp → accepted_qty if it exists
        if "accepted_qty_mrp" in items_df.columns:
            items_df = items_df.rename(columns={"accepted_qty_mrp": "accepted_qty"})
        
        # Ensure accepted_qty is numeric
        if "accepted_qty" in items_df.columns:
            items_df["accepted_qty"] = pd.to_numeric(items_df["accepted_qty"], errors="coerce")
        ...
        # Reorder columns
        cols_order = list(metadata.keys()) + ["s_no", "article", "item_description", "uom", "challan_qty", "received_qty", "accepted_qty"]
        cols_order = [col for col in cols_order if col in items_df.columns]
        items_df = items_df[cols_order]
        
        # Convert to list of dictionaries
        rows = items_df.to_dict('records')
        
        return rows
    
    def _get_value(self, data, possible_keys, default=""):
        """Return the first found key value from dict, handling nested keys."""
...
def main():
    st.title("⚡ Reliance GRN Automation Dashboard")
    ...
    if 'pdf_config' not in st.session_state:
        st.session_state.pdf_config = {
            ...
            'llama_api_key': "llx-0BwTH88VqmXI4cljr2FAj6CHJIDSHzhAwllNSkaNWtZkKwb6",
            'llama_agent': "Reliance",
    ...
    st.sidebar.info("Configure settings above, then authenticate and choose a workflow to run")
    
    # Authentication button
    st.header("Authentication")
    auth_button = st.button("Authenticate with Google", use_container_width=True)
    ...
    if st.session_state.workflow_state['authenticated']:
    ...
    if not st.session_state.workflow_state['type'] and not st.session_state.workflow_state['running'] and not st.session_state.workflow_state['authenticated']:
        ...
        st.info("Configure your settings in the sidebar, authenticate, then select a workflow above to begin automation")
    
    # Run workflows using session state configurations
    if st.session_state.workflow_state['type'] and not st.session_state.workflow_state['running'] and st.session_state.workflow_state['authenticated']:
        # Create automation instance
        automation = RelianceAutomation()
        
        # Re-authenticate to ensure services are initialized
        if automation.authenticate_from_secrets(auth_progress, auth_status):
